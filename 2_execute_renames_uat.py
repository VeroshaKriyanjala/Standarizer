...
def parse_rename_plan(plan_file):
    """Parse uat_rename_plan.md and extract only the Planned Renames."""
    renames = []
    ...
    with open(plan_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if in_planned_section and "UAT/" in line:
                # Table format: | `UAT/old_name` | `UAT/new_name` |
                parts = line.split("|")
                if len(parts) >= 3:
                    old_cell = parts[1].strip().strip("`").strip()
                    new_cell = parts[2].strip().strip("`").strip()
                    if old_cell.startswith("UAT/") and new_cell.startswith("UAT/"):
                        old_rel = old_cell.replace("UAT/", "", 1)
                        new_rel = new_cell.replace("UAT/", "", 1)
                        if old_rel != new_rel:
...
def execute_for_repo(repo_name, repo_path, dry_run=False):
    plan_file = OUTPUT_DIR / repo_name / "uat_rename_plan.md"
    uat_dir = Path(repo_path) / "UAT"

    if not plan_file.exists():
        print(f"  uat_rename_plan.md not found. Run 1_plan_prod_renames_uat.py first.")
        return
    ...
    if dry_run:
        ...
        for old_rel, new_rel in renames:
            print(f"    UAT/{old_rel} -> UAT/{new_rel}")
        return
    ...
    # Step 2: Execute renames in UAT on the new branch
    ...
    for old_rel, new_rel in renames:
        old_path = uat_dir / old_rel
        new_path = uat_dir / new_rel
    ...
    # Write execution log
    log_file = OUTPUT_DIR / repo_name / "uat_rename_execution_log.md"
    (OUTPUT_DIR / repo_name).mkdir(parents=True, exist_ok=True)
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"# UAT Rename Execution Log - {repo_name}\n\n")
        f.write(f"Branch: `{BRANCH_NAME}`\n")
        f.write(f"Source: `{uat_dir}`\n\n")
