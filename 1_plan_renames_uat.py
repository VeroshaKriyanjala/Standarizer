...
def get_uat_services(uat_dir: Path) -> dict:
    """Get all module/service pairs in UAT. Returns {relative_path: absolute_path}."""
    services = {}
...
def get_git_renames_uat(repo_path: str) -> dict:
    """
    Discover UAT folder renames from git history.
...
def plan_for_repo(repo_name, repo_path, rename_map, final_mappings):
    ...
    output_file = out_dir / "uat_rename_plan.md"

    if not uat_dir.exists():
        print(f"  UAT directory not found at {uat_dir}")
        return

    # Get current UAT services
    uat_services = get_uat_services(uat_dir)
    print(f"  Found {len(uat_services)} services in UAT")

    # Get git rename history for UAT
    git_renames = get_git_renames_uat(repo_path)
    print(f"  Found {len(git_renames)} folder renames in UAT git history")
    ...
    for old_name, new_name in sorted(rename_map.items()):
        if old_name in uat_services:
            planned_renames.append((old_name, new_name))
        elif new_name in uat_services:
            already_correct.append((old_name, new_name))
        else:
            not_in_uat.append((old_name, new_name))

    # --- Section 2: Git history analysis (matches, git-only, mismatches) ---
    # Build final.md mappings with UAT/ prefix for comparison
    # Maps {uat_current: [uat_previous_list]}
    final_uat_mappings = defaultdict(list)
    for current, previous_list in final_mappings.items():
        # final.md already uses UAT/ prefix, so use as-is
        for previous in previous_list:
            final_uat_mappings[current].append(previous)

    matches = []
    mismatches = []
    git_only = []

    all_dirs = set(git_renames.keys()) | set(final_uat_mappings.keys())
    for current_dir in sorted(all_dirs):
        in_git = current_dir in git_renames
        in_final = current_dir in final_uat_mappings

        if in_git and in_final:
            git_prev = git_renames[current_dir]
            final_prev_list = final_uat_mappings[current_dir]
    ...
    print(f"  Not in UAT: {len(not_in_uat)}")
    print(f"  Git matches: {len(matches)}")
    print(f"  Git-only renames: {len(git_only)}")
    print(f"  Mismatches: {len(mismatches)}")

    # Find unaffected UAT folders
    all_old = set(rename_map.keys())
    all_new = set(rename_map.values())
    unaffected = sorted([s for s in uat_services if s not in all_old and s not in all_new])

    # Write report
    out_dir.mkdir(parents=True, exist_ok=True)
    with open(output_file, "w", encoding="utf-8") as f:
        f.write(f"# UAT Folder Rename Plan - {repo_name}\n\n")
        f.write("Based on UAT rename history (`final.md`) and UAT git history.\n\n")

        # Planned Renames
        f.write(f"## Planned Renames ({len(planned_renames)})\n\n")
        f.write("Old name found in UAT, will be renamed to new standard name.\n\n")
        if planned_renames:
            f.write("| Old Name (in UAT) | New Name (standard) |\n")
            f.write("|---|---|\n")
            for prev, curr in planned_renames:
                f.write(f"| `UAT/{prev}` | `UAT/{curr}` |\n")
        else:
        ...
        f.write("UAT already has the new standard name.\n\n")
        if already_correct:
            f.write("| Service Name |\n")
            f.write("|---|\n")
            for _, new_name in already_correct:
                f.write(f"| `UAT/{new_name}` |\n")
        else:
            f.write("None.\n")
        f.write("\n")

        # Not in UAT
        f.write(f"## Not in UAT ({len(not_in_uat)})\n\n")
        f.write("Neither old nor new name exists in this repo's UAT.\n\n")
        if not_in_uat:
            f.write("| Old Name | Expected New Name |\n")
            f.write("|---|---|\n")
            for prev, curr in not_in_uat:
                f.write(f"| `{prev}` | `{curr}` |\n")
        else:
        ...
        f.write("UAT git rename history **agrees** with `final.md`.\n\n")
        if matches:
        ...
        f.write("Renames found in UAT git history but **NOT** in `final.md`.\n\n")
        if git_only:
            f.write("| Current Folder | Previous Name |\n")
            f.write("|---|---|\n")
            for m in git_only:
        ...
        # Unaffected
        f.write(f"## Unaffected UAT Folders ({len(unaffected)})\n\n")
        f.write("Exist in UAT but have no matching rename in `final.md`. May need manual review.\n\n")
        if unaffected:
            for p in unaffected:
                f.write(f"- `UAT/{p}`\n")
        else:
