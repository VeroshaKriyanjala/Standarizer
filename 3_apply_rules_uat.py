...
def find_matching_deployment_path(uat_rel_path, deploy_folders):
    """Find the matching DEPLOYMENT-BASE-REPO path for a UAT service folder.

    uat_rel_path: e.g. "pharmacy/csi-phr-base"
    deploy_folders: set of paths in DEPLOYMENT-BASE-REPO

    Returns the matching path or None.
    """
    # Direct match
    if uat_rel_path in deploy_folders:
        return uat_rel_path

    # Try matching by service name (last component) within the same module
    parts = uat_rel_path.split("/")
    if len(parts) >= 2:
        module = parts[0]
        service = parts[-1]
...
def execute_for_repo(repo_name, repo_path, deploy_map, deploy_folders, dry_run=False):
    """Process all service folders in a repo's UAT directory."""
    ...
    for pattern in ["kustomization.yaml", "hpa.yaml", "patch-hpa.yaml", "patch-pod.yaml"]:
        for f in uat_dir.rglob(pattern):
    ...
    for service_dir in service_dirs:
        uat_rel_path = str(service_dir.relative_to(uat_dir))
        deploy_path = find_matching_deployment_path(uat_rel_path, deploy_folders)
        deployment_name = deploy_map.get(deploy_path) if deploy_path else None
        service_info[service_dir] = (uat_rel_path, deploy_path, deployment_name)
    ...
    rule1_changes = 0
    for service_dir in service_dirs:
        uat_rel_path, deploy_path, deployment_name = service_info[service_dir]
        ...
        if new_content != content:
            if not dry_run:
                with open(kust_file, "w", encoding="utf-8") as f:
                    f.write(new_content)
            print(f"    [{uat_rel_path}] Updated base URL")
            log_entries.append((uat_rel_path, "Rule 1: Updated base URL to DEPLOYMENT-BASE-REPO"))
            rule1_changes += 1
    ...
    for service_dir in service_dirs:
        ...
        if new_content != content:
            if not dry_run:
                with open(kust_file, "w", encoding="utf-8") as f:
                    f.write(new_content)
            print(f"    [{uat_rel_path}] Updated target name to '{deployment_name}'")
            log_entries.append((uat_rel_path, f"Rule 3: Updated patchesJson6902 Deployment target name to '{deployment_name}'"))
            rule3_changes += 1
    ...
    for service_dir in service_dirs:
        ...
        for yaml_name in ["hpa.yaml", "patch-hpa.yaml", "patch-pod.yaml"]:
            ...
            if yaml_file.exists():
                ...
                if changed:
                    print(f"    [{uat_rel_path}] {yaml_name} - {msg}")
                    log_entries.append((uat_rel_path, f"Rule 4: {yaml_name} - {msg}"))
                    rule4_changes += 1
    ...
    # Write log
    log_file = OUTPUT_DIR / repo_name / "more_rules_uat_log.md"
    (OUTPUT_DIR / repo_name).mkdir(parents=True, exist_ok=True)
    with open(log_file, "w", encoding="utf-8") as f:
        f.write(f"# More Rules (UAT) Execution Log - {repo_name}\n\n")
