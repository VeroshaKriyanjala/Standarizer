...
# DEPLOYMENT-BASE-REPO has no PROD folder - services are at root level
REPOS_WITHOUT_PROD = {"DEPLOYMENT-BASE-REPO"}


def get_prod_services(repo_name, repo_path):
    """Get all module/service pairs (depth=2 subfolders only)."""
    if repo_name in REPOS_WITHOUT_PROD:
        base_dir = Path(repo_path)
    else:
        base_dir = Path(repo_path) / "PROD"
...
def main():
    ...
    for repo_name, repo_path in DEFAULT_REPOS.items():
        repo_services[repo_name] = get_prod_services(repo_name, repo_path)
        source = "root" if repo_name in REPOS_WITHOUT_PROD else "PROD"
    ...
    output_file = OUTPUT_DIR / "all_repos_prod_comparison.md"
    with open(output_file, "w", encoding="utf-8") as f:
        f.write("# PROD Service Names - All Repos Comparison\n\n")
        f.write(f"Compares current PROD subfolder names across all {total_repos} repos.\n\n")
        f.write("| Symbol | Meaning |\n")
        f.write("|---|---|\n")
        f.write("| Y | Exists with this exact name |\n")
        f.write("| - | Does not exist |\n\n")
        f.write(f"**Repos:** {', '.join(REPO_NAMES)}\n\n")
        f.write(f"*Note: DEPLOYMENT-BASE-REPO uses root level (no PROD folder)*\n\n")
