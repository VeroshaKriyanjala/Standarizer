...
REPO_NAMES = ["DEPLOYMENT-BASE-REPO"]
...
def get_deployment_name(directory: Path) -> str:
    """Extract metadata.name from Deployment.yaml if it exists in the directory."""
    dep_file = directory / "Deployment.yaml"
...
def print_tree(directory: Path, prefix: str = "", is_last: bool = True, file=sys.stdout):
    ...
    dep_name = get_deployment_name(directory)
    if dep_name:
        print(f"{prefix}{connector}{directory.name}/  [deployment: {dep_name}]", file=file)
    else:
...
def main():
    ...
    for repo_name, repo_path in repos.items():
        ...
        if repo_name in REPOS_WITHOUT_UAT:
            ...
            output_file = out_dir / "nofiles_structure_with deployments.md"
