...
Checks version conditions in each repo's UAT and replaces Sync-new/Sync-DM/Sync
...
BRANCH_NAME = "UAT-dm-initiator-update"
...
def check_version_conditions(repo_path):
    ...
    base_kust = uat_dir / "base" / "base-utility-service-java-sev" / "kustomization.yaml"
    base_tag = get_image_tag(base_kust)

    # Find csi-iam-service kustomization
    iam_kust = uat_dir / "security" / "csi-iam-service" / "kustomization.yaml"
...
def process_repo(repo_name, repo_path, deploy_map, onering_names, dry_run=False):
    ...
    if not uat_dir.exists():
        print(f"  UAT directory not found")
        return False, []
    ...
    # Find all kustomization files in UAT
    kust_files = sorted(uat_dir.rglob("kustomization.yaml"))
    ...
    for kust_file in kust_files:
        rel_path = str(kust_file.parent.relative_to(uat_dir))
    ...
    if total_changes > 0 and not dry_run:
        try:
            ...
            commit_msg = (
                f"Add DM-Initiator and replace Sync references in UAT\n\n"
...
def write_log(repo_name, log_entries, conditions_met, base_tag, iam_tag, dry_run):
    ...
    log_file = log_dir / "dm_initiator_log_uat.md"
